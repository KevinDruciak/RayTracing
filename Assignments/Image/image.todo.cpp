...
Pixel32 Image32::nearestSample( Point2D p ) const
{
	...
	Point2D p1 = Point2D(p[0] * (double)this->_width, p[1] * (double)this->_height);
	//std::cout << p1[0] << " " << p1[1] << std::endl;
	//std::cout <<  " " << std::endl;

	if (!checkBounds(floor(p1[0] + 0.5), _width) || !checkBounds(floor(p1[1] + 0.5), _height)) {
		newPix.a = 0;
		newPix.r = 0;//(*this)(floor(p1[0] + 0.5), floor(p1[1] + 0.5)).r;
		newPix.b = 0;//(*this)(floor(p1[0] + 0.5), floor(p1[1] + 0.5)).b;
		newPix.g = 0;//(*this)(floor(p1[0] + 0.5), floor(p1[1] + 0.5)).g;
	} else {
		newPix.a = (*this)(floor(p1[0] + 0.5), floor(p1[1] + 0.5)).a;
		newPix.r = (*this)(floor(p1[0] + 0.5), floor(p1[1] + 0.5)).r;
		newPix.b = (*this)(floor(p1[0] + 0.5), floor(p1[1] + 0.5)).b;
		newPix.g = (*this)(floor(p1[0] + 0.5), floor(p1[1] + 0.5)).g;
	}
